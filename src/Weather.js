...
class Weather extends React.Component {
  ...
  search = (prevState) => {
    if (this.refs.myInput !== null) {
      ...
      fetch(
        `https://api.openweathermap.org/data/2.5/weather?q=${inputValue}&appid=${api.key}`
      )
        .then((res) => res.json())
        .then((result) => {
          this.setState({
            ...
            <main>
              ...
              <div className="current">
                ...
                <div className="hi-low">{`(${
                  parseInt(this.state.data.main.temp_min) - 273
                }) °C / (${
                  parseInt(this.state.data.main.temp_max) - 273
                }) °C`}</div>
              </div>
            </main>
          )}
